  ...
  "en-US": {
    translation: {
      ...
      cli: {
        ...
        directoryPrompt: "Working directory (optional, blank = bridge cwd)",
        ...
        queueInsertInvalidDuration:
          "Invalid value for {{flag}}: \"{{value}}\". {{detail}}",
        ...
        queueRetried:
          'Task {{taskId}} of queue "{{name}}" re-queued (state: pending) — it will be consumed on the next tick.',
        queueRetryTaskNotFound: 'Task "{{taskId}}" not found in queue "{{name}}".',
        queueRetryNotFailed:
          'Task "{{taskId}}" of queue "{{name}}" is not failed (state: {{state}}) — only failed tasks can be retried.',
        taskNamePrompt: "Task name",
        taskNameInvalid:
          "Task name must be [a-z0-9-]+ (lowercase letters, digits and hyphens only)",
        taskNameExists: 'A task named "{{name}}" already exists.',
        schedulePrompt: "Schedule (examples: {{examples}})",
        scheduleInvalid: "{{reason}}. Examples: {{examples}}",
        timeoutPrompt: "Timeout (default 5h)",
        silencePrompt: "Silence window (default 30m)",
        modelPlaceholder: "Example: azure-openai-responses/gpt-5.6-terra",
        bodyPrompt: "Shared context body (optional, blank = empty; prepended to every task prompt)",
        taskCreated: "Created successfully!",
        taskCreatedGuideFile: "Edit {{filePath}} to set your prompt.",
        taskCreatedGuideTarget:
          "Send `/schedule-here {{name}}` in chat app to set the report place.",
        scheduleRunTriggered:
          'Scheduled task "{{name}}" triggered — the run result will be delivered to the task\'s target chat.',
        scheduleRunWaitNotSettled:
          'Scheduled task "{{name}}" was triggered, but the CLI stopped waiting before the run reached an endpoint ({{reason}}). The run may still be in progress or was interrupted — check `agent-bridge schedule history` for the actual outcome.',
        scheduleRunWaitInterrupted: "interrupted by user (SIGINT)",
        scheduleRunWaitTimeout: "wait exceeded the task's timeout bound",
        scheduleRunFailed: 'Failed to trigger scheduled task "{{name}}": {{reason}}',
        scheduleRunTaskNotFound: 'No scheduled task "{{name}}" found.',
        scheduleRunNoChannels:
          'No channels are configured — create one with "agent-bridge add" first.',
        scheduleRunChannelNotFound:
          'Scheduled task "{{name}}" is bound to channel "{{channel}}", but no channel with that name is configured.',
      },
      schedule: {
      ...
      queue: {
        ...
        taskFailedAgentSession: "Agent session: {{sessionId}}",
      },
    },
  },
  "zh-CN": {
    translation: {
      ...
      cli: {
        ...
        directoryPrompt: "工作目录（可选，留空使用 bridge 启动目录）",
        queueCreated: "创建成功！",
        ...
        queueInsertInvalidDuration: "{{flag}} 的值无效：\"{{value}}\"。{{detail}}",
        queueInsertUnboundWarning:
          "警告：该队列尚未绑定目标聊天——任务将一直等待，直到通过 `/queue-here` 绑定。",
        queueNotFound: '未找到队列 "{{name}}"。',
        noQueues: "尚未创建任何队列。请使用 `agent-bridge queue add` 创建。",
        queueRetried:
          '已将队列 "{{name}}" 的任务 {{taskId}} 重新入队（state: pending），将由下个 tick 自然消费。',
        queueRetryTaskNotFound: '队列 "{{name}}" 中未找到任务 "{{taskId}}"。',
        queueRetryNotFailed:
          '队列 "{{name}}" 的任务 "{{taskId}}" 未处于 failed 状态（当前状态：{{state}}）——只有 failed 任务可以重试。',
        taskNamePrompt: "任务名称",
        taskNameInvalid: "任务名称只能由小写字母、数字和连字符组成（[a-z0-9-]+）",
        taskNameExists: '已存在名为 "{{name}}" 的任务。',
        schedulePrompt: "定时表达式（示例：{{examples}}）",
        scheduleInvalid: "{{reason}}。示例：{{examples}}",
        timeoutPrompt: "超时时间（默认 5h）",
        silencePrompt: "静默窗口（默认 30m）",
        modelPlaceholder: "示例：azure-openai-responses/gpt-5.6-terra",
        bodyPrompt: "共享上下文（可选，留空为空；会拼接在每个任务 prompt 之前）",
        taskCreated: "创建成功！",
        taskCreatedGuideFile: "编辑 {{filePath}} 设置任务 prompt。",
        taskCreatedGuideTarget: "在聊天应用中发送 `/schedule-here {{name}}` 设置结果投递的聊天。",
        scheduleRunTriggered: '定时任务 "{{name}}" 已触发，运行结果将投递到该任务的目标聊天。',
        scheduleRunWaitNotSettled:
          '定时任务 "{{name}}" 已触发，但 CLI 在运行到达终点前停止了等待（{{reason}}）。运行可能仍在进行或已被中断——请用 `agent-bridge schedule history` 查看实际结果。',
        scheduleRunWaitInterrupted: "被用户中断（SIGINT）",
        scheduleRunWaitTimeout: "等待超过了任务的超时上界",
        scheduleRunFailed: '触发定时任务 "{{name}}" 失败：{{reason}}',
        scheduleRunTaskNotFound: '未找到定时任务 "{{name}}"。',
        scheduleRunNoChannels: '尚未配置任何 channel——请先使用 "agent-bridge add" 创建。',
        scheduleRunChannelNotFound:
          '定时任务 "{{name}}" 绑定的 channel "{{channel}}" 不存在或未配置。',
      },
      schedule: {
      ...
      queue: {
        ...
        taskFailedAgentSession: "Agent 会话：{{sessionId}}",
      },
    },
  },
} as const;
